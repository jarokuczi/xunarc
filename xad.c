    ...
    ULONG ret = RETURN_FAIL, numerr = 0;
        ...
        if(config->dst)
        {
            ...
            if((ai = (struct xadArchiveInfo *)
                ...
                FPrintf(config->output, "xadGetInfo returned %d code\n", err);
                while(!err && loop)
                {
                    ...
                    FPrintf(config->output, "Processing file %s with size %d bytes\n", fi->xfi_FileName, fi->xfi_Size);

                    while(fi && !(SetSignal(0L,0L) & SIGBREAKF_CTRL_C) && !xh.finish) {

                        if(!config->src || CheckName(config->src,  fi->xfi_FileName))
                        {
...
ULONG progrhook(struct Hook *hook,void *o, struct xadProgressInfo *pi)
{
    FPrintf(config->output, "Hook called with %s\n",pi->xpi_FileName);
return 0;
}
...
LONG CheckNameSize(STRPTR name, ULONG size)
{
    ...
    if((r = strlen(name)) > size)
    {
        ...
        FPrintf(config->output, "\r\033[KFilename '%s' exceeds name limit of %ld by %ld, rename? (Y|\033[1mN\033[0m|Q): ", name, size, r-size);
